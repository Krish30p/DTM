...
const StudentDashboard = ({ loggedInUser, studentSelections, setCurrentView, handleLogout }) => {
    const studentId = loggedInUser.id;
    // New: Access the attendance data for the logged-in student
    const studentAttendance = studentSelections[studentId]?.attendance || [];
...
<br />
                            ...
                            <table className="w-full border-collapse text-left">
                                <thead className="bg-gray-100">
                                    <tr>
                                        ...
                                        <th className="border p-3 text-center">Total Classes</th>
                                ...
                                <tbody>
                                    {studentAttendance.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50">
                                            <td className="border p-3 font-medium">{item.subject}</td>
                                            <td className="border p-3 text-center">{item.total}</td>
                                            <td className="border p-3 text-center">{item.present}</td>
                                            <td className={`border p-3 text-center ${getStatusColor(item.percentage)}`}>
                ...
                {/* --- END NEW ATTENDANCE SECTION --- */}

            </div>
        </div>
    );
};
// --- END UPDATED STUDENT DASHBOARD COMPONENT ---


const StaffDashboard = ({ loggedInUser, studentSelections, setCurrentView, handleLogout }) => {
    return (
...
const App = () => {
    ...
    if (currentView === 'student-dashboard') return (
        <StudentDashboard
    ...
    if (currentView === 'staff-dashboard') return (
        <StaffDashboard
            loggedInUser={loggedInUser}
            studentSelections={studentSelections}
            setCurrentView={setCurrentView}
